...
const StudentManagement = () => {
  ...
  const generateSampleStudents = () => {
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      sampleData.push({
        ...
        token: `TOKEN${i}${Math.random().toString(36).substring(2, 5).toUpperCase()}`
      });
    }
  ...
  const handleBatchUpload = (csvData) => {
    ...
    const newStudents = lines
      ...
      .map(line => {
        const [studentNumber, firstName, lastName, department] = line.split(',');
  ...
  return (
    <div className="flex flex-col h-full max-h-full overflow-hidden p-4">
      ...
      {/* Tab Navigation */}
      <div className="flex-none flex border-b border-gray-300 mb-2">
        <button
        ...
        <button
          className={`py-1 px-4 font-medium flex items-center justify-center ${
            activeTab === 'form' 
              ? 'text-[#38438c] border-b-2 border-[#38438c]' 
